...
function estimateShiliaoOrganPosterHeight(data) {
  const L = getShiliaoOrganPosterLayout();
  const innerW = SHILIAO_POSTER_W - L.PAD * 2 - L.INNER_X * 2;
  const canvas = document.createElement('canvas');
  const ctx = canvas.getContext('2d');
  const { principleH, ingredientsH, dishesH } = measureOrganPosterSections(ctx, data, innerW);
...
function renderShiliaoOrganPoster(data) {
  ...
  const H = estimateShiliaoOrganPosterHeight(data);

  const label = document.createElement('div');
  label.className = 'poster-page-label';
  label.textContent = `${data.name} · 器官食补海报`;
  ...
  const ctx = canvas.getContext('2d');
  ctx.scale(dpr, dpr);
  drawShiliaoOrganPoster(ctx, data, W, H);
}
...
function drawShiliaoOrganPoster(ctx, data, W, H) {
  ...
  ctx.fillText(`${data.icon || ''} ${data.name} · 器官食补`, W / 2, y + L.HEADER_INNER * 0.36);
  ...
  drawSection('偏爱食材', ingredientsH, (w) => {
    let cy = 0;
