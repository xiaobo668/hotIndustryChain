/**
 * 2026 锂电池板块产能排行榜复验（10 个赛道）
 * 运行: node scripts/verify-capacity-rank-battery2026.js
 */
const fs = require('fs');
const vm = require('vm');
const path = require('path');
const { RANKINGS } = require('./build-capacity-rank-battery2026');
...
const rankCode = fs.readFileSync(
  path.join(root, 'data', 'capacity-rank-battery2026.js'),
  'utf8'
);
const sandbox = {};
vm.runInNewContext(
  rankCode + '\n' + chainCode + '\nexports={INDUSTRY_DATA,CAPACITY_RANK_REGISTRY_BATTERY2026};',
  sandbox
);

const INDUSTRY_DATA = sandbox.exports.INDUSTRY_DATA;
const REGISTRY = sandbox.exports.CAPACITY_RANK_REGISTRY_BATTERY2026;

const STRONG_BY_KEY = {
  动力电池: /动力|电芯|GWh|电池|储能|PACK/i,
  正极材料: /正极|前驱体|LFP|NCM|磷酸铁锂|三元|钴酸锂/i,
  负极材料: /负极|石墨|人造石墨|阳极|石墨化/i,
  电解液: /电解液|LiFSI|六氟|锂盐|溶剂/i,
  隔膜: /隔膜|涂覆|湿法|干法/i,
  铜箔: /铜箔|极薄|极耳/i,
  固态电池: /固态|硫化物|氧化物|电解质|半固态/i,
  电池设备: /设备|产线|涂布|卷绕|叠片|装配|交付|检测/i,
  储能电池: /储能|大储|工商业|GWh|户储/i,
  电池回收: /回收|梯次|再生|拆解|资源化/i,
};
