/**
 * 光互联（光学链接）订单榜复验
 * 运行: node scripts/verify-order-rank-optical-interconnect2026.js
 */
const fs = require('fs');
const vm = require('vm');
const path = require('path');
const { ENTRIES } = require('./build-order-rank-optical-interconnect2026');
...
const rankCode = fs.readFileSync(
  path.join(root, 'data', 'order-rank-optical-interconnect2026.js'), 'utf8'
);
const sandbox = {};
vm.runInNewContext(
  rankCode + '\n' + chainCode + '\nexports={ORDER_RANK_OPTICAL_INTERCONNECT2026,INDUSTRY_DATA};',
  sandbox
);

const data = sandbox.exports.ORDER_RANK_OPTICAL_INTERCONNECT2026;
const chain = sandbox.exports.INDUSTRY_DATA['光互联'];
...
if (data.key !== '光互联') errors.push('key 应为 光互联');
...
byRank.forEach((co) => {
  ...
  if (!chainNames.has(co.name)) warnings.push(`不在光互联产业链节点: ${co.name}`);
  if (co.rank > ORDER_RANK_MAX && (co.verify.note.includes('营收') || co.verify.note.includes('规模参考') || co.verify.sourceType === 'official')) {
    warnings.push(`${co.name}: 7-10位以2025年报营收/媒体订单作参考，非统一在手订单口径`);
  }
  if (co.rank <= ORDER_RANK_MAX && co.verify.sourceType === 'media' && !co.verify.officialCross) {
    warnings.push(`${co.name}: 1-6位为媒体报道口径，建议关注公司后续公告验证`);
  }
  if (co.name === '光迅科技' && co.verify.officialCross && co.verify.amount > co.verify.officialCross.exact) {
    warnings.push(`${co.name}: 媒体估算65亿与官方合同负债10.95亿口径不一致，已登记 officialCross`);
  }
  if (co.name === '烽火通信' && co.verify.note.includes('光传输')) {
    warnings.push(`${co.name}: 订单口径含光传输/算力设备，非纯光模块`);
  }
});

console.log('=== 光互联订单榜复验 ===');
