/**
 * MLCC 订单榜复验
 * 运行: node scripts/verify-order-rank-mlcc2026.js
 */
const fs = require('fs');
const vm = require('vm');
const path = require('path');
const { ENTRIES } = require('./build-order-rank-mlcc2026');
...
const rankCode = fs.readFileSync(
  path.join(root, 'data', 'order-rank-mlcc2026.js'), 'utf8'
);
const sandbox = {};
vm.runInNewContext(
  rankCode + '\n' + chainCode + '\nexports={ORDER_RANK_MLCC2026,INDUSTRY_DATA};',
  sandbox
);

const data = sandbox.exports.ORDER_RANK_MLCC2026;
const chain = sandbox.exports.INDUSTRY_DATA['MLCC'];
...
byRank.forEach((co) => {
  ...
  if (!chainNames.has(co.name)) warnings.push(`不在MLCC产业链节点: ${co.name}`);
