/**
 * 人形机器人订单榜复验
 * 运行: node scripts/verify-order-rank-humanoid-robot2026.js
 */
const fs = require('fs');
const vm = require('vm');
const path = require('path');
const { ENTRIES } = require('./build-order-rank-humanoid-robot2026');
...
const rankCode = fs.readFileSync(
  path.join(root, 'data', 'order-rank-humanoid-robot2026.js'), 'utf8'
);
const sandbox = {};
vm.runInNewContext(
  rankCode + '\n' + chainCode + '\nexports={ORDER_RANK_HUMANOID_ROBOT2026,INDUSTRY_DATA};',
  sandbox
);

const data = sandbox.exports.ORDER_RANK_HUMANOID_ROBOT2026;
const chain = sandbox.exports.INDUSTRY_DATA['机器人'];
...
byRank.forEach((co) => {
  ...
  if (!chainNames.has(co.name)) warnings.push(`不在机器人产业链节点: ${co.name}`);
  if (co.rank > ORDER_RANK_MAX && co.verify.note.includes('营收')) {
    warnings.push(`${co.name}: 7-10位以2025年报业务规模作参考，非统一在手订单口径`);
  }
