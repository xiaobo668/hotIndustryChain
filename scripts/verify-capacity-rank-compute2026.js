/**
 * 2026 算力板块产能排行榜复验（10 个赛道）
 * 运行: node scripts/verify-capacity-rank-compute2026.js
 */
const fs = require('fs');
const vm = require('vm');
const path = require('path');
const { RANKINGS } = require('./build-capacity-rank-compute2026');
...
const rankCode = fs.readFileSync(
  path.join(root, 'data', 'capacity-rank-compute2026.js'),
  'utf8'
);
const sandbox = {};
vm.runInNewContext(
  rankCode + '\n' + chainCode + '\nexports={INDUSTRY_DATA,CAPACITY_RANK_REGISTRY_COMPUTE2026};',
  sandbox
);

const INDUSTRY_DATA = sandbox.exports.INDUSTRY_DATA;
const REGISTRY = sandbox.exports.CAPACITY_RANK_REGISTRY_COMPUTE2026;

const STRONG_BY_KEY = {
  算力芯片: /算力|GPU|AI芯片|芯片|SoC|推理|视觉/i,
  AI服务器: /AI服务器|智算|服务器|ODM|整机/i,
  量子计算: /量子|光量子|量子通信|量子安全|光器件/i,
  算力租赁: /算力|智算|GPU|租赁|IDC/i,
  大数据: /大数据|金融|数据|月活|MAU|平台/i,
  液冷技术: /液冷|冷板|浸没|散热|冷却/i,
  云计算: /云计算|IDC|机架|数据中心|托管/i,
  边缘计算: /边缘|CDN|IoT|物联网|节点|模组/i,
  算力算法: /算法|大模型|AI|API|调用|视觉|NLP/i,
  PCB算力板: /PCB|算力板|服务器|交换机|覆铜板/i,
};
