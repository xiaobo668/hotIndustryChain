/**
 * 2026 消费电子产能排行榜复验（6 个赛道）
 * 运行: node scripts/verify-capacity-rank-consumer-electronics2026.js
 */
const fs = require('fs');
const vm = require('vm');
const path = require('path');
const { RANKINGS } = require('./build-capacity-rank-consumer-electronics2026');
...
const rankCode = fs.readFileSync(
  path.join(root, 'data', 'capacity-rank-consumer-electronics2026.js'),
  'utf8'
);
const sandbox = {};
vm.runInNewContext(
  rankCode + '\n' + chainCode + '\nexports={INDUSTRY_DATA,CAPACITY_RANK_REGISTRY_CONSUMER_ELECTRONICS2026};',
  sandbox
);

const INDUSTRY_DATA = sandbox.exports.INDUSTRY_DATA;
const REGISTRY = sandbox.exports.CAPACITY_RANK_REGISTRY_CONSUMER_ELECTRONICS2026;

const STRONG_BY_KEY = {
  显示面板: /面板|显示|OLED|LCD|AMOLED|模组/i,
  TWS耳机音箱: /TWS|耳机|声学|音箱|AirPods|电声/i,
  智能手机ODM: /手机|ODM|EMS|整机|智能.*终端/i,
  可穿戴设备: /可穿戴|穿戴|手表|手环|Watch|VR|AR/i,
  充电设备: /充电|电源|快充|移动电源|适配器|氮化镓/i,
  摄像头模组: /摄像头|摄像|光学|镜头|模组|影像/i,
};
