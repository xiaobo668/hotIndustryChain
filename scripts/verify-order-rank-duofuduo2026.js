/**
 * 多氟多产业链订单榜复验
 * 运行: node scripts/verify-order-rank-duofuduo2026.js
 */
const fs = require('fs');
const vm = require('vm');
const path = require('path');
const { ENTRIES } = require('./build-order-rank-duofuduo2026');
...
const rankCode = fs.readFileSync(
  path.join(root, 'data', 'order-rank-duofuduo2026.js'), 'utf8'
);
const sandbox = {};
vm.runInNewContext(
  rankCode + '\n' + chainCode + '\nexports={ORDER_RANK_DUOFUDUO2026,INDUSTRY_DATA};',
  sandbox
);

const data = sandbox.exports.ORDER_RANK_DUOFUDUO2026;
const chain = sandbox.exports.INDUSTRY_DATA['多氟多'];
...
byRank.forEach((co) => {
  ...
  if (!chainNames.has(co.name)) warnings.push(`不在多氟多产业链节点: ${co.name}`);
  ...
  if (co.name === '多氟多' && co.verify.note.includes('估算')) {
    warnings.push(`${co.name}: 六氟磷酸锂订单按出货量与均价估算，非公司单独披露金额`);
  }
});

console.log('=== 多氟多产业链订单榜复验 ===');
