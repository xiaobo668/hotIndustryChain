...
const { writeWechatIndex } = require('./lib/wechat-article');
...
function pushReport(config) {
  const result = writeAnalysisReport(config);
  entries.push(result.manifestEntry);
  wechatEntries.push(result.wechatManifestEntry);
}
...
const wechatManifestPath = writeWechatIndex(wechatEntries);

console.log('OK 分析文档目录:', manifestPath);
console.log('OK 公众号文稿目录:', wechatManifestPath);
console.log('OK 入口页: analysis.html / wechat.html');
entries.forEach((e) => console.log('  -', e.title, '->', e.html));
wechatEntries.forEach((e) => console.log('  📱', e.articleTitle, '->', e.html));
