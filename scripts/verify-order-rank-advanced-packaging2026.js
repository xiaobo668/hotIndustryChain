/**
 * 先进封装订单榜复验
 * 运行: node scripts/verify-order-rank-advanced-packaging2026.js
 */
const fs = require('fs');
const vm = require('vm');
const path = require('path');
const { ENTRIES } = require('./build-order-rank-advanced-packaging2026');
...
const rankCode = fs.readFileSync(
  path.join(root, 'data', 'order-rank-advanced-packaging2026.js'), 'utf8'
);
const sandbox = {};
vm.runInNewContext(
  rankCode + '\n' + chainCode + '\nexports={ORDER_RANK_ADVANCED_PACKAGING2026,INDUSTRY_DATA};',
  sandbox
);

const data = sandbox.exports.ORDER_RANK_ADVANCED_PACKAGING2026;
const chain = sandbox.exports.INDUSTRY_DATA['先进封装'];
...
byRank.forEach((co) => {
  ...
  if (!chainNames.has(co.name)) warnings.push(`不在先进封装产业链节点: ${co.name}`);
  if (co.rank > ORDER_RANK_MAX && (co.verify.note.includes('营收') || co.verify.note.includes('规模参考'))) {
    warnings.push(`${co.name}: 7-10位以2025年报营收/业务规模作参考，非在手订单`);
  }
  if (co.rank === 1 && co.verify.officialCross) {
    warnings.push(`${co.name}: 报道320亿与行业测算90-160亿口径并存，已在 officialCross 登记`);
  }
  if (co.name === '佰维存储') {
    warnings.push(`${co.name}: 科创板先进封测标的，产业链节点未收录`);
  }
});

console.log('=== 先进封装订单榜复验 ===');
